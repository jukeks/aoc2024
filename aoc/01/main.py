from aoc.lib import create_matrix, print_matrix, parse_matrix, Matrix, Any

test_input = parse_matrix(
    """
two1nine
eightwothree
abcone2threexyz
...
def find_first_and_last_number_working(row: str) -> int:
...
def find_first_and_last_number(row: str) -> int:
    matches: list[tuple[int, int]] = []

    for number in numbers:
        last_idx = 0
        while True:
            idx = row.find(number, last_idx)
            if idx == -1:
                break
            print(row[last_idx:], last_idx)
            matches.append((idx, numbers[number]))
            last_idx = idx + 1
...
def main():
    s = 0
    for row in test_input2:
        row = "".join(row)
        print(row)
        n = find_first_and_last_number(row)
        n2 = find_first_and_last_number_working(row)
        if n != n2:
            breakpoint()
        print(n)
        s += n
